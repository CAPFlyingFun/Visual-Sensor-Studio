...
test('the state store is the one owner, and flow is one-directional', () => {
  ...
  const outside = code.replace(fn[0], '');
...
test('Night stacks at the size the TIER chose, not the size the screen is', () => {
  ...
  // THE POINT IS THE `undefined` IN THE THIRD ARGUMENT, not the shape of the
  // fourth: a still must not advance the live camera's temporal state. The
...
test('Night measures its own result, then lifts it, then saves what it lifted', () => {
  ...
  const photoTs = readFileSync(new URL('../src/v2/capture/photo.ts', import.meta.url), 'utf8');
  assert.match(photoTs, /if \(!options\.preRendered\s*\n?\s*&& !renderStill\(/,
    'the upload AND the re-render are behind one guard, so a pre-rendered '
    + 'save touches neither');
