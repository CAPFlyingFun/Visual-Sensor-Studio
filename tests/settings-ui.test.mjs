...
test('settings version is 0.4.3 everywhere it is stated', () => {
  assert.match(htmlSource, /Visual Sensor Studio v0\.4\.3/);
  assert.match(mainSource, /APP_VERSION\s*=\s*['"]0\.4\.3['"]/);
  assert.match(swSource, /visual-sensor-studio-v0\.4\.3/);
});
