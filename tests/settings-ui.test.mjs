...
test('settings version is 0.24.0 everywhere it is stated', () => {
  assert.match(htmlSource, /Visual Sensor Studio v0\.24\.0/);
  assert.match(mainSource, /APP_VERSION\s*=\s*['"]0\.24\.0['"]/);
  assert.match(swSource, /visual-sensor-studio-v0\.24\.0/);
});
