...
test('Survey ships, and Blueprint is the one that is actually blue', () => {
  // It shipped as "Blueprint" and had no blue in it — cyan and amber. The
  // name went to the lens that earns it and this one says what it does.
  const survey = STARTER_LENSES.find((lens) => lens.id === 'lens-v2-survey');
  assert.ok(survey, 'Survey is a starter');
  assert.equal(survey.name, 'Survey');
  assert.ok(survey.kinds.strength > 0 && survey.kinds.depth > 0);
  assert.match(survey.note, /text and closed contours need to be traced, not measured/,
    'the missing kind is named rather than quietly absent');

  const blueprint = STARTER_LENSES.find((lens) => lens.id === 'lens-v2-blueprint');
  assert.ok(blueprint.grid && blueprint.grid.strength > 0, 'Blueprint is the ruled one');
  assert.equal(blueprint.kinds, undefined, 'and it is not also the kinds lens');
  assert.ok(blueprint.stops.every((stop) =>
    parseInt(stop.color.slice(5, 7), 16) > parseInt(stop.color.slice(1, 3), 16)),
  'every stop is bluer than it is red — the name is now true');
  assert.match(blueprint.note, /measure nothing absolute/,
    'a ruled grid still measures nothing without a known distance');
});

test('no compiled lens uses a GLSL reserved word — every optional feature on', () => {
  /*
   * THE SCAN EXISTED AND DID NOT COVER THIS PATH.
   *
   * v2-pipeline checks every built-in FILTER for reserved words, because a
   * shader is a string to TypeScript: one builds, tests, deploys, and dies on
   * the device with a blank preview. 'half' reached a review that way once
   * (Poly, 2026-09-04) — and then reached one AGAIN here, in the grid's line
   * width, because compiled lenses were never scanned. Most of the shader
   * text this app runs is now generated by compileLens rather than written in
   * the registry, so the check has to follow it.
   *
   * Every optional block is turned on at once, or a word hiding in one that
   * the starters happen not to use would still get through.
   */
  const RESERVED = ['half', 'flat', 'long', 'short', 'double', 'union', 'input',
    'output', 'this', 'class', 'template', 'namespace', 'sizeof', 'cast'];
  const everything = sanitiseLens({
    ...BLUE_OUTLINE,
    fill: FILL,
    shapeHue: 1,
    kinds: { strength: 0.9, depth: 0.5 },
    grid: { cells: 12, strength: 0.6 }
  });
  const lenses = [everything, ...STARTER_LENSES];
  for (const lens of lenses) {
    const filter = compileLens(lens);
    // Comments only ever mention reserved words; the compiler does not read them.
    const code = filter.fragment
      .replace(/\/\*[\s\S]*?\*\//g, ' ')
      .replace(/\/\/[^\n]*/g, ' ');
    for (const reserved of RESERVED) {
      assert.ok(!new RegExp(`\\b${reserved}\\b`).test(code),
        `${lens.name}: '${reserved}' is reserved in GLSL ES 1.00`);
    }
  }
});

test('the grid rules SQUARE cells, counted on the short side', () => {
  const lens = sanitiseLens({ ...BLUE_OUTLINE, grid: { cells: 5, strength: 0.6 } });
  assert.deepEqual(lens.grid, { cells: 5, strength: 0.6 });
  // Whole cells only, and inside the offered range.
  assert.equal(sanitiseLens({ ...BLUE_OUTLINE, grid: { cells: 12.7, strength: 1 } }).grid.cells, 13);
  assert.equal(sanitiseLens({ ...BLUE_OUTLINE, grid: { cells: 99, strength: 1 } }).grid.cells, 30);
  assert.equal(sanitiseLens({ ...BLUE_OUTLINE, grid: { cells: 1, strength: 1 } }).grid.cells, 5);
  assert.equal(sanitiseLens({ ...BLUE_OUTLINE, grid: { cells: 12, strength: 0 } }).grid, undefined);

  const code = compileLens(lens).fragment;
  /*
   * THE COUNT IS ON THE SHORT SIDE, and that is what keeps the cells square.
   * Joshua worked it out himself: 4:3 at five across is 6.667 down, so the
   * long side takes SIX whole cells and the remaining two thirds becomes an
   * equal margin at each end. A grid stretched to fit both sides would put a
   * different distance in a horizontal cell than a vertical one, and
   * comparing across the picture — the only thing it is good for — would lie.
   */
  assert.match(code, /float cell = min\(size\.x, size\.y\) \/ GRID_CELLS;/);
  assert.match(code, /vec2 count = floor\(size \/ cell/);
  assert.match(code, /vec2 margin = \(size - count \* cell\) \* 0\.5;/);

  // Frame-relative width, or a 3024-wide still would be ruled with hairlines
  // where the preview was ruled boldly.
  assert.match(code, /min\(size\.x, size\.y\) \* 0\.0009/);

  // ADDED OVER the finished picture, never blended into it: a ruling must not
  // take anything away from what it is ruling over.
  assert.match(code, /c = max\(c, texture2D\(uRamp, vec2\(1\.0, 0\.5\)\)\.rgb \* gridRule\(vUv\) \* GRID_STRENGTH\)/);

  // Unlike the fill and the kinds, a ruling makes sense over any output mode.
  for (const output of ['mask', 'swap']) {
    assert.ok(compileLens(sanitiseLens({ ...BLUE_OUTLINE, output, grid: lens.grid }))
      .fragment.includes('gridRule'), `${output} can still be ruled`);
  }
  assert.notEqual(compileLens(sanitiseLens(BLUE_OUTLINE)).revision, compileLens(lens).revision);
});
