...
test('THE CONTROL THAT DECIDES WHAT THIS IS: sharpening is separated from merging', () => {
  ...
  const report = bm.mergeAndCompare(capture(scene, SPREAD), 8);
...
test('a burst that adds nothing is reported as adding nothing', () => {
  // The verdict has to survive the case where the honest answer is "no".
  const clustered = bm.mergeAndCompare(capture(scene, CLUSTERED), 8);
...
test('detail beyond the single-frame Nyquist is the claim that cannot be faked', () => {
...
test('the control is built from the same frame everything aligned to', () => {
...
test('one usable frame is not dressed up as a merge', () => {
  const single = bm.mergeAndCompare(capture(scene, [{ shiftX: 0, shiftY: 0 }]), 8);
  assert.equal(single.splat, null);
  assert.equal(single.best, 'control');
  assert.match(single.verdict, /nothing to merge/);
});

test('a scene with no edge yields no number, and says so', () => {
  ...
  const report = bm.mergeAndCompare(capture(textured, SPREAD), 8);
...
test('the blur inverted is derived from the picture, not assumed', () => {
  ...
  const report = bm.mergeAndCompare(capture(scene, SPREAD, { psf: 1.6 }), 8);
  const sharper = bm.mergeAndCompare(capture(scene, SPREAD, { psf: 0.6 }), 8);
...
test('the comparison strip carries every version at the same size', () => {
  const report = bm.mergeAndCompare(capture(scene, SPREAD), 8);
