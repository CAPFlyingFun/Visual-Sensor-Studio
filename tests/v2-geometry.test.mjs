...
test('a maximum-tier filtered clip records the CHOSEN stream, risk stated up front (fake device)',
  { skip: runnable ? false : 'no browser available' }, async () => {
    ...
    await withBrowser(async (browser, base) => {
      ...
      // renderer. MEASURED 2026-09-03 — this test passes run on its own and
      // times out when the file's 25 browser tests run back to back, on an
      // unchanged tree as readily as a changed one. The assertion is
      // untouched; only the time allowed for it to become true is, because
      // the claim is "the row says this", not "within three seconds".
      await page.waitForFunction(() =>
        /RECORDING · filtered render/.test(document.getElementById('v2DiagRecordIn')?.textContent ?? ''),
        null, { timeout: 15000 });
      const during = await page.evaluate(() => ({
