...
test('Milestone B: the GPU pipeline renders truthfully (fake device)',
  { skip: runnable ? false : 'no browser available' }, async () => {
    ...
    await withBrowser(async (browser, base) => {
      ...
      assert.deepEqual(boot.filters.slice(0, 17),
        ['rgb', 'ironbow', 'difference', 'motion', 'speed', 'trails', 'chrono',
          'slitscan', 'edges', 'grid', 'poly', 'cel', 'ink', 'wash', 'amplify',
          'flow', 'background'],
...
test('Milestone E: the lens workbench edits a live custom lens with exact numbers, saves, imports (fake device)',
  { skip: runnable ? false : 'no browser available' }, async () => {
    await withBrowser(async (browser, base) => {
      ...
      const STARTERS = ['Survey', 'Cloud Sort', 'Cloud Storm', 'Blueprint', 'Prism', 'Blue Antenna', 'Coloring Book Style', 'Colour Splash', 'Colour Hide',
...
test('a fault on the device says what it was, and otherwise stays out of the way (fake device)',
  { skip: runnable ? false : 'no browser available' }, async () => {
    await withBrowser(async (browser, base) => {
      ...
      // A CLEAN BOOT SAYS NOTHING, and leaves nothing in the document. A
      // diagnostic that might itself be a suspect is worth nothing, so it
      // adds no markup at all until it has something to report.
      assert.equal(await page.evaluate(() =>
        document.getElementById('v2Fault')), null,
      'nothing is wrong, so there is nothing in the page');
      ...
      const shown = await page.evaluate(() => ({
        present: !!document.getElementById('v2Fault'),
        text: document.getElementById('v2FaultText')?.textContent ?? ''
      }));
      assert.equal(shown.present, true, 'a real fault is not silent');
      ...
      // AND IT CAN BE PUT AWAY, which takes it back out of the document.
